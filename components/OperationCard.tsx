...
const OPERATION_PRIZE_CONTEXT: Record<string, { prizeLabel: string; teaser: string }> = {
  'canton-weekend-1': {
    ...
    teaser: 'The full three-path Canton Quests experience — Family, Challenge, or Secret. One citywide leaderboard.',
  },
  'fair-qr-hunt': {
...
export default function OperationCard({ event, status }: OperationCardProps) {
  ...
  return (
    <article
      ...
      style={{
        borderColor: isLive ? 'rgba(34, 211, 238, 0.4)' : 'rgba(217, 164, 76, 0.35)',
        background: isLive
          ? 'linear-gradient(160deg, rgba(34,211,238,0.08), rgba(5,6,7,0.94))'
          : 'linear-gradient(160deg, rgba(217,164,76,0.08), rgba(5,6,7,0.94))',
      }}
    >
      <div className="flex items-center justify-between gap-2">
        <span
          ...
          style={{
            color: isLive ? '#22d3ee' : '#f0c978',
            borderColor: isLive ? 'rgba(34,211,238,0.5)' : 'rgba(240,201,120,0.5)',
            backgroundColor: isLive ? 'rgba(34,211,238,0.12)' : 'rgba(240,201,120,0.1)',
          }}
        >
          <span
