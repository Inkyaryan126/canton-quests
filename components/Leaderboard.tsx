...
export default function Leaderboard({ entries, currentPlayerId }: LeaderboardProps) {
  if (entries.length === 0) {
    return (
      <div className="glass-panel p-8 text-center text-gray-400 font-mono text-sm">
        No scores recorded yet in this event. Be the first agent to complete a quest!
      </div>
    );
  }
  ...
  return (
    <div className="glass-panel overflow-hidden border-amber-500/20">
      <div className="p-4 bg-obsidian/60 border-b border-[var(--border-subtle)] flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            🏆 Event Leaderboard
          </h2>
          <p className="text-xs text-gray-400 font-mono">Live ranking based on verified quest completions</p>
        </div>
        <span className="badge badge-medium">{entries.length} Competitors</span>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  className={`text-xs font-mono font-bold px-2.5 py-1 rounded-lg border ${rankColor}`}
                >
                  {rankBadge}
                </span>
