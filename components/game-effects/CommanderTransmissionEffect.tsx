...
/** Signal-decode beat before content reveals — near-instant under reduced motion. */
const DECODE_MS = 650;
...
export default function CommanderTransmissionEffect({ moment, onDismiss, reducedMotion = false }: CommanderTransmissionEffectProps) {
  ...
  useEffect(() => {
    setPhase('decoding');
    const timer = setTimeout(() => setPhase('revealed'), reducedMotion ? DECODE_MS_REDUCED : DECODE_MS);
    ...
    >
      <div className={`relative z-10 ${isPortrait ? 'max-w-sm' : 'max-w-lg'} w-full max-h-[90vh] overflow-y-auto bg-[#07090e]/97 border-2 border-amber-500/60 rounded-2xl shadow-[0_0_60px_rgba(245,158,11,0.25)]`}>
        ...
        <div className="relative">
          ...
          {phase === 'decoding' && (
            <div className="absolute inset-0 rounded-b-2xl flex flex-col items-center justify-center gap-3 py-14 bg-[#07090e]">
