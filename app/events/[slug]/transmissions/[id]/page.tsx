...
/** Signal-decode beat before an archived transmission's card reveals — mirrors CommanderTransmissionEffect's arrival treatment. */
const DECODE_MS = 650;
const DECODE_MS_REDUCED = 150;
...
function TransmissionPlayerPageContent({ params }: { params: { slug: string; id: string } }) {
  ...
  useEffect(() => {
    if (state !== 'unlocked' || !transmission) return;
    setRevealPhase('decoding');
    const timer = setTimeout(() => setRevealPhase('revealed'), reducedMotion ? DECODE_MS_REDUCED : DECODE_MS);
    ...
    return (
      <div className="min-h-screen bg-stone-950 text-stone-100 flex flex-col selection:bg-amber-500 selection:text-stone-950 font-body">
        ...
        <main className="flex-1 flex justify-center items-center px-4 py-16">
          <TransmissionLoader label="CHECKING SIGNAL" reducedMotion={reducedMotion} />
        </main>
        <CinematicFooter />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 flex flex-col selection:bg-amber-500 selection:text-stone-950 font-body">
      ...
      <main className="flex-1 flex justify-center">
        <div className="relative w-full max-w-3xl px-4 py-10 sm:py-14 flex flex-col items-center">
          <CqTransition
            show={revealPhase === 'revealed'}
