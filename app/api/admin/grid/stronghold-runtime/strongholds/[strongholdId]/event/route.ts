...
import { setGridNpcStrongholdRuntimeAdminEvent } from '@/lib/grid/server/npc-stronghold-runtime-admin-service';
...
export async function POST(
  ...
  try {
    ...
    if (typeof body.active !== 'boolean') {
      return noStore(
        { success: false, error: 'Invalid Grid NPC runtime request: active must be boolean' },
        { status: 400 },
      );
    }
    const idempotencyKey =
      typeof body.idempotencyKey === 'string' ? body.idempotencyKey : '';
    const result = await setGridNpcStrongholdRuntimeAdminEvent(
    ...
    if (!invalid) console.error('[Grid admin stronghold runtime event POST]', error);
    return noStore(
      { success: false, error: invalid ? message : 'Failed to set Grid NPC stronghold event state.' },
