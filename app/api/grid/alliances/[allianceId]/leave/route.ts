...
import { leaveGridAllianceMembership } from '@/lib/grid/server/alliance-persistence-service';
...
export async function POST(request: Request, { params }: { params: { allianceId: string } }) {
  ...
  try {
    ...
    const membership = await leaveGridAllianceMembership(createSupabaseGridAlliancePersistencePort(), {
      allianceId: params.allianceId,
      seasonId: context.seasonId,
      playerId: session.player.id,
      now: new Date().toISOString(),
    }, context.rules);
    return response({ success: true, membership });
  } catch (error) {
    return response({ success: false, error: error instanceof Error ? error.message : 'Failed to leave Grid Alliance.' }, { status: 400 });
  }
}
