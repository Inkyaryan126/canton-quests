...
export async function POST(request: Request, { params }: { params: { channelId: string } }) {
  ...
  try {
    await createSupabaseGridChatPort().leaveParty(params.channelId, context.playerId, new Date().toISOString());
    return gridChatJson(context.session, { success: true, left: true });
  } catch (error) {
