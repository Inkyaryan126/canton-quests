...
export async function POST(request: Request, { params }: { params: { channelId: string } }) {
  ...
  try {
    await createSupabaseGridChatPort().leaveRoom(params.channelId, context.playerId, new Date().toISOString());
    return gridChatJson(context.session, { success: true, channelId: params.channelId, left: true });
  } catch (error) {
