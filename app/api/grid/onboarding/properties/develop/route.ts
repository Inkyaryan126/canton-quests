...
import { developGridOnboardingProperty } from '@/lib/grid/server/onboarding-property-service';
...
export async function POST(request: Request) {
  ...
  if (!propertySlug.trim() || !idempotencyKey.trim() || !branch) {
    return response(
      {
        success: false,
        error: 'Missing propertySlug, branch, or idempotencyKey.',
      },
      { status: 400 },
    );
  }
  try {
    const property = await developGridOnboardingProperty(
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Failed to develop Grid onboarding property.';
