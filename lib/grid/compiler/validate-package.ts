...
// Plan §5.4 specifies that GEOMETRY_INVALID / TERRITORY_OUT_OF_BOUNDS /
// OVERLAP_TOLERANCE_EXCEEDED reuse lib/grid/geo/geometry.ts (Compiler 2).
// That module has not landed on this branch yet (both landing attempts
// failed), and this task's write scope does not include lib/grid/geo, so
// the small amount of polygon math those three checks need is implemented
// locally below instead. This duplication should be removed in favor of the
// shared lib/grid/geo utilities once that module lands.
...
function ringIsSimple(ring: Ring): boolean {
  // Ring is closed (last point repeats the first); walk the distinct edges only.
...
function isValidPolygonGeometry(geom: GeoJSON.MultiPolygon): boolean {
...
function pointInMultiPolygon(point: GeoJSON.Position, geom: GeoJSON.MultiPolygon): boolean {
  for (const polygon of geom.coordinates) {
    let inside = false;
    for (const ring of polygon) {
...
function ringVertices(ring: Ring): GeoJSON.Position[] {
  // Drop the closing duplicate of the first point.
  return ring.slice(0, -1);
}

function isWithinBounds(geom: GeoJSON.MultiPolygon, bounds: GeoJSON.MultiPolygon): boolean {
  for (const polygon of geom.coordinates) {
    for (const ring of polygon) {
...
// Sutherland-Hodgman clip of `subject` against convex `clip` ring. Fixture
// geometries used by this validator's tests are simple convex rectangles,
// which this handles exactly.
...
export function runCityValidation(
  ...
  for (const edge of pkg.edges) {
    ...
    // A GridCityPackage only carries the territories of a single city, so a
    // territory slug on an edge that isn't part of this package's own
    // territories can only have come from a different city's geography.
  ...
  for (const landmark of pkg.landmarks) {
    // GridLandmarkDefinition has no publicNameSafe field -- a landmark's
    // name is always shown to players, so any residential/private landmark
  ...
  const allSlugs = new Set<string>([...territorySlugs, ...pkg.properties.map((p) => p.slug), ...pkg.landmarks.map((l) => l.slug)]);
