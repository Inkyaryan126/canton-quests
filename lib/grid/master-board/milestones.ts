...
export const GRID_MILESTONES: GridMilestoneDefinition[] = [
  ...
  { id: 'market', title: 'Property Market', phase: 'economy', dependsOn: ['economy-core'], lanePatterns: ['market-auction-ui'], branchPatterns: ['grid-market-'], integrationCommitSignals: ['GRID Market 5', 'GRID Market 4'] },
  { id: 'auctions', title: 'Auctions', phase: 'economy', dependsOn: ['market'], lanePatterns: ['market-auction-ui'], branchPatterns: ['grid-auction-'], integrationCommitSignals: ['GRID Auction 3'] },
];
GRID_MILESTONES.push(
  ...
  { id: 'takeover', title: 'Takeover Persistence', phase: 'gameplay', dependsOn: ['contest-system'], lanePatterns: ['takeover-persistence'], branchPatterns: ['grid-takeover'], integrationCommitSignals: ['takeover damage'] },
  { id: 'return-experience', title: 'Return Briefing', phase: 'player', dependsOn: ['onboarding'], lanePatterns: ['return-ui'], branchPatterns: ['grid-return'], integrationCommitSignals: ['return summary', 'return briefing'] },
...
GRID_MILESTONES.push(
  ...
  { id: 'admin-control', title: 'GM / Admin Control Center', phase: 'operations', dependsOn: ['onboarding'], lanePatterns: ['admin-control-center'], branchPatterns: ['grid-admin-control-center'], integrationCommitSignals: ['Grid admin control center', 'GRID Admin'] },
  { id: 'launch-readiness', title: 'Launch QA & Simulation', phase: 'operations', dependsOn: ['onboarding'], lanePatterns: ['launch-readiness'], branchPatterns: ['grid-launch-readiness'], integrationCommitSignals: ['launch readiness'] },
