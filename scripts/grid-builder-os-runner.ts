...
function supervisorPrompt(assignments: CrewDispatchAssignment[]): string {
  ...
  return [
    ...
    '- Claim lifecycle: `grid:agents claim` creates a 2-minute startup reservation. After a real worker process exists, immediately run `npm run grid:agents -- activate --lane <lane> --pid <pid>`. Never heartbeat a reservation.',
    ...
    '- Derive exact file scope for each selected task first. Create all safe non-overlapping Control Tower startup reservations and isolated worktrees sequentially BEFORE starting concurrent worker execution. If scopes cannot be proven non-overlapping, do not parallelize those tasks.',
    '- A reservation is NOT an active worker. For the Codex self-work slot, activate its lane with the live lead PID before editing. For Claude/Gemini, capture `$!` when the background process starts and immediately activate that lane with that PID. If launch fails, release the reservation immediately. Untouched reservations that never activate are automatically reaped after two minutes.',
    ...
    '- Launch Claude and Gemini as background processes after their reservations exist, capture each PID/log separately, activate each reservation with that live PID, then immediately begin the activated Codex slot task. Do not wait for one worker to finish before starting the others.',
