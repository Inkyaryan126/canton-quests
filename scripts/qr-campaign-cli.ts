...
import { generateCampaignFlyers } from '../lib/qr-flyer-generator';
...
Generate Promotional Master Flyers:
  npm run qr:campaign -- flyers --campaign "Canton Quests Street Team 2026" --masters ~/Desktop/Quest_Flyers --output ~/Desktop/Quest_Flyers/Canton_Quests_Street_Team_2026_Generated
...
export async function runQrCampaignCli(argv: string[], io: CliIo = DEFAULT_IO): Promise<number> {
  ...
  if (command === 'flyers') {
    if (flags.masters || flags.output) {
      const summary = await generateCampaignFlyers({
        campaignIdentifier: requireString(flags, 'campaign'),
        mastersDir: requireString(flags, 'masters'),
        outputDir: requireString(flags, 'output'),
      });
      io.stdout('===== FLYER GENERATION COMPLETE =====');
      io.stdout(`Campaign: ${summary.campaignName}`);
      io.stdout(`Active QR assignments: ${summary.activeAssignmentsCount}`);
      io.stdout(`Flyers generated: ${summary.flyersGeneratedCount}`);
      io.stdout(`Family: ${summary.countsByType.family}`);
      io.stdout(`Challenge: ${summary.countsByType.challenge}`);
      io.stdout(`Secret: ${summary.countsByType.secret}`);
      io.stdout(`Output: ${summary.outputDirectory}`);
      return 0;
    }
    await listCampaignChildren('flyers', requireString(flags, 'campaign'), io);
    return 0;
  }

  if (command === 'distributors' || command === 'qrs') {
