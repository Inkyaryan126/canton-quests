...
| **12. Vote Count Limits** | Configure votes per session (strictly `max_votes_per_session = 1`). | Enforced by `uq_spectator_one_vote_per_event` constraint, `check_spectator_vote_limit` trigger, and `cast_spectator_vote` RPC. | `audience_events` (`max_votes_per_session = 1`), `audience_votes` |
...
- **Database-Enforced Single Vote Limits**: In addition to `cast_spectator_vote` RPC validation, the database executes a PostgreSQL `BEFORE INSERT` trigger (`trg_enforce_spectator_vote_limit`) verifying `existing_votes < 1`. The unique constraint `CONSTRAINT uq_spectator_one_vote_per_event UNIQUE (audience_event_id, session_token_hash)` guarantees strict uniqueness per session hash per event and prevents duplicate voting or race conditions under high concurrent volume.
