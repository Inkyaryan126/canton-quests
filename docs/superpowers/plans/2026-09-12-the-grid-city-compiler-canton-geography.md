...
- **This plan does not build era-gating, release schedules, or any gameplay-visible historical sequencing.** The design brief for "PAST → PRESENT → PLAYER-CREATED FUTURE" (era-based unlocking) is explicitly a later gameplay-layer concern (§13) — this phase only ensures the data model can carry the metadata without retrofitting the schema or the `GridCityPackage` shape later. No `era` enum is fixed here beyond an open `string` field, deliberately, because locking specific Canton-history era boundaries now would violate the "do not lock exact Canton date ranges without historical evidence" instruction.
